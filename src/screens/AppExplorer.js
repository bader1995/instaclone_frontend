...
export const AppExplorer = () => {
    ...
    const getPosts = () => {
        ...
        get(ref(db, 'instaclone/files/pictures'))
        .then((snapshot) => {
    ...
    return (
        <>
            ...
            <div style={mainContainer}>
                <AppTopBar customeStyle={{ zIndex: 1 }} reLoadData={() => getPosts()} />
                { loading && <img src={loaderPic} width={50} style={{ alignSelf: "center", marginTop: 100, marginBottom: -50 }} /> }
                <div style={contentContainer}>
                    ...
                    {
                        showData && 
                        posts.map((item) => <PostComponent url={item.url} id={item.id} key={item.key} />)
                    }
